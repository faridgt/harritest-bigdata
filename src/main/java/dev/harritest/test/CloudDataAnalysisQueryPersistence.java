...
public class CloudDataAnalysisQueryPersistence  implements DataAnalysisQueryPersistence {
		...
		public void hitAnalysisAndPersist() {
			...
			for (String c:cnts) {
			  ...
			  maxall.show();
			  maxall.write().option("header", true).mode(SaveMode.Overwrite).parquet(AppConf.CLOUD_DEST_ANALYSIS_PATH+"club_h_valuable_players.parquet");
			...
			for (String c:cnts) {
			  ...
			  maxall_final.show();
			  maxall.write().option("header", true).mode(SaveMode.Overwrite).parquet(AppConf.CLOUD_DEST_ANALYSIS_PATH+"/europe_or_America_best_players.parquet");
